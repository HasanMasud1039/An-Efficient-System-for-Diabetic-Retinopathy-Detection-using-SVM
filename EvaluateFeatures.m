...
Entropy1 = entropy(segmented_images{1});
Entropy2 = entropy(segmented_images{2});
Entropy3 = entropy(segmented_images{3});
figure,subplot(2,3,2);imshow(I);title('Original Image');
subplot(2,3,4);imshow(segmented_images{1});title("Cluster 1, Entropy "+Entropy1); 
subplot(2,3,5);imshow(segmented_images{2});title("Cluster 2, Entropy "+Entropy2); 
subplot(2,3,6);imshow(segmented_images{3});title("Cluster 3, Entropy "+Entropy3); 
set(gcf, 'Position', get(0,'Screensize'));
set(gcf, 'name','Segmented by K Means', 'numbertitle','off')

% Feature Extraction
x = inputdlg('Enter the cluster no. :');
i = str2double(x);
...
feat_disease = [Mean, Standard_Deviation, Entropy, RMS, Variance, Smoothness, Kurtosis, Skewness, IDM,Contrast,Correlation,Energy,Homogeneity];